...
if __name__ == '__main__':
	...
	url = 'http://i.imgur.com/X017qGH.jpg'
	img = np.array(PIL.Image.open(urllib.request.urlopen(url)))[:,:,0]
	...
	"*** YOUR CODE HERE ***"
	img_copy = np.ndarray.flatten(img)
	shuffle_img = np.random.permutation(img_copy)
	"*** END YOUR CODE HERE ***"
	...
	for index in range(len(rank_list)):
		...
		"*** YOUR CODE HERE ***"
		# used solutions to help me with this function
		reconstructed = U[:, :k] @ np.diag(S)[:k, :k] @ V[:k, :]
