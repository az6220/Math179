...
def NLL(X, y, W, reg=0.0):
	...
	eta = X @ W
	exp_eta = np.exp(eta)
	u = exp_eta / exp_eta.sum(axis=1).reshape(-1,1) # reshape to only one column
	reg_term = reg * np.diag(W.T @ W).sum() 
	# took this from solutions, not sure where it comes form
	NLL = - ((y * np.log(u)).sum(axis=1).sum()) + reg_term
	"*** END YOUR CODE HERE ***"
	return NLL
...
def grad_softmax(X, y, W, reg=0.0):
	...
	eta = X @ W
	exp_eta = np.exp(eta)
	u = exp_eta / exp_eta.sum(axis=1).reshape(-1,1)
	grad = X.T @ (u - y) + W * reg
	"*** END YOUR CODE HERE ***"
	return grad
...
def predict(X, W):
	...
	eta = X @ W
	exp_eta = np.exp(eta)
	prob = exp_eta / exp_eta.sum(axis=1).reshape(-1,1) 
	# probability matrix according to softmax equation
...
def grad_descent(X, y, reg=0.0, lr=1e-5, eps=1e-6, max_iter=500, print_freq=20):
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		if np.isnan(nll):
			break
		nll_list.append(nll)
		gradient = grad_softmax(X, y, W, reg=reg)
		W = W - lr * gradient
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
	...
	for reg in lambda_list:
		...
		"*** YOUR CODE HERE ***"
		W_opt, nll_list = grad_descent(X_train, y_train_OH, reg=reg, lr=2e-5, print_freq=50)
	...
	"*** YOUR CODE HERE ***"
	opt_reg_index = np.argmax(accu_list)
	reg_opt = lambda_list[opt_reg_index]
	"*** END YOUR CODE HERE ***"
