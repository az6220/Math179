...
def grad_logreg(X, y, W, reg=0.0):
	...
	"""
	#Find the gradient of logistic regression with respect to W
	"*** YOUR CODE HERE ***"
	u = sigmoid(X @ W)
	grad = X.T @ (u - y) + reg * W 
	# Murphy p. 252 gradient takes form of g(w) + lambda * w
	"*** END YOUR CODE HERE ***"
	return grad
...
def NLL(X, y, W, reg=0.0):
	...
	"""
	# Find the negative log likelihood of logistic regression
	"*** YOUR CODE HERE ***"
	u = sigmoid(X @ W)
	l2_reg = reg / 2 * np.linalg.norm(W) ** 2 # why divide reg by 2? 
	nll = -(y * np.log(u) + (1 - y) * np.log(1 - u)).sum() + l2_reg
	"*** END YOUR CODE HERE ***"
	return nll
...
def grad_descent(X, y, reg=0.0, lr=1e-4, eps=1e-6, max_iter=500, print_freq=20):
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		"*** YOUR CODE HERE ***"
		# Step 1: Calculate NLL and append to nll_list
		nll = NLL(X, y, W, reg)
		if np.isnan(nll):
			break
		nll_list.append(nll)
...
def newton_step(X, y, W, reg=0.0):
	...
	grad = grad_logreg(X, y, W, reg)
	mu = sigmoid(X @ W)
	diag = np.diag(np.squeeze(np.asarray((mu * (1-mu)))))
	# diag = np.diag(mu * (1-mu))
	reg_term = reg * np.eye(X.shape[1])
	H = X.T @ diag @ X + reg_term
...
def newton_method(X, y, reg=0.0, eps=1e-6, max_iter=20, print_freq=5):
	...
	while iter_num < max_iter and np.linalg.norm(step) > eps:
		...
		if np.isnan(nll):
			break
		nll_list.append(nll)
		grad_step = newton_step(X, y, W, reg=reg)
		W = W - grad_step
		"*** END YOUR CODE HERE ***"
...
def get_description(X, y, W):
	...
	"""
	# Find the accuracy, precision, recall, and f-1 score of the prediction
	"*** YOUR CODE HERE ***"
	pred = predict(X, W)
	m = y.shape[0]
	temp = np.zeros((m,2))
	temp[:,0] = pred[:,0]
	temp[:,1] = y[:,0]
	accuracy_arr = temp[temp[:,0] == temp[:,1]]
	accuracy = accuracy_arr.shape[0] / m
	precision_arr = accuracy_arr[accuracy_arr[:,0]==1]
	precision_tot = temp[temp[:,0]==1].shape[0]
	precision = precision_arr.shape[0] / precision_tot
	recall_arr = accuracy_arr[accuracy_arr[:,1]==1]
	recall_tot = temp[temp[:,1]==1].shape[0]
	recall = recall_arr.shape[0] / recall_tot
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	# taken suggested reg terms from solution set 
	a_list, p_list, r_list, f1_list = [], [], [], []
	for index,reg in enumerate(reg_list):
		W_opt, nll_list = grad_descent(X_train, y_train, reg = reg, \
			lr = 2e-4, print_freq = 100)
			# how do we know to increase learning rate?
	...
	"*** YOUR CODE HERE ***"
	max_acc_index = np.argmax(a_list)
	reg_opt = reg_list[max_acc_index]
	"*** END YOUR CODE HERE ***"
	return reg_opt
